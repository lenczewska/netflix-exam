...
const HoverCardM = ({ data, onAdd, onMouseEnter, onMouseLeave, handleMoreInfo, onOpenModal, isFavorite }) => {
  if (!data || Object.keys(data).length === 0) return null;
  const releaseYear = data.release_date ? new Date(data.release_date).getFullYear() : "N/A";
  return (
    <div className="hover-card-movie absolute z-9990 bg-[#111] text-[#fff] w-[330px] h-[380px] p-[9px] top-[500px] -translate-x-1/2 shadow-2xl transition-all duration-300">
      {data.backdrop_path && (
        <img src={`https://image.tmdb.org/t/p/w500${data.backdrop_path}`} className="rounded mb-2 hover-card-movie-img" alt={data.title || data.name} />
      )}
      <h3 className="text-xl font-bold mb-1 line-clamp-1 hover-card-movie-title">{data.title || data.name}</h3>
      <p className="text-sm mb-2 hover-card-movie-desc">{releaseYear} | {data.vote_average ? data.vote_average.toFixed(1) : "N/A"}</p>
