...
function Modal({ game, onClose }) {
  ...
  return (
    <>
      {/* Оверлей */}
      <div
        className="fixed inset-0 bg-[#2b272762] bg-opacity-100 z-100"
        onClick={onClose}
      ></div>

      {/* Модалка */}
      <div className=" w-max inset-0  left-[20px] flex justify-center items-center z-150">
        <div className="modal-box relative max-w-[1200px] w-[90%] bg-[#181818] rounded-[10px] p-[20px]">
          {/* Кнопка-крестик */}
          <button
            onClick={onClose}
            className="absolute top-[15px] right-[15px] text-white text-[22px] hover:text-red-500"
          >
